...
export default function Login() {
  ...
  async function submit(event) { event.preventDefault(); setError(''); setLoading(true); try { const data = await authApi.login({ email: identifier, password }); localStorage.setItem('routesetu-token', data.access_token); window.dispatchEvent(new Event('routesetu-auth-changed')); navigate('/dashboard'); } catch (requestError) { setError(requestError.message); } finally { setLoading(false); } }
